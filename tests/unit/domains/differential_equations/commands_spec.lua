...
describe("Tungsten Differential Equations Commands", function()
    ...
    before_each(function()
        ...
        mock_selection.get_visual_selection = spy.new(function() return current_selection_text end)
        ...
        _G.require = function(module_path)
            if module_path == "tungsten.util.selection" then return mock_selection end
            if module_path == "tungsten.util.logger" then return mock_logger end
            if module_path == "tungsten.core.parser" then return mock_parser end
            if module_path == "tungsten.core.engine" then return mock_evaluator end
            if module_path == "tungsten.util.insert_result" then return mock_insert_result end
            if module_path == "tungsten.config" then return mock_config end
            if module_path == "tungsten.core.ast" then return mock_ast end
    ...
    test_command(
        function() de_commands.solve_ode_command() end,
        ":TungstenSolveODE",
        "y' = y",
        { type = "ode" },
        function(parsed_ast)
    ...
    test_command(
        function() de_commands.wronskian_command() end,
        ":TungstenWronskian",
        "W(f,g)",
        { type = "wronskian" },
    ...
    test_command(
        function() de_commands.laplace_command() end,
        ":TungstenLaplace",
        "f(t)",
        { type = "function_call" },
        function(ast) return mock_ast.create_laplace_transform_node(ast) end
    )

    test_command(
        function() de_commands.inverse_laplace_command() end,
        ":TungstenInverseLaplace",
        "F(s)",
        { type = "function_call" },
        function(ast) return mock_ast.create_inverse_laplace_transform_node(ast) end
    )

    test_command(
        function() de_commands.convolve_command() end,
        ":TungstenConvolve",
        "f(t) * g(t)",
        { type = "convolution" },
        function(ast) return ast end
