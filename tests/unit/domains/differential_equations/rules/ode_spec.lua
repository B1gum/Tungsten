...
describe("Differential Equations ODE Rule", function()
  ...
  before_each(function()
    local mock_tk = {
      ...
      rbrace = P("}"),
      equals_op = P("="),
      variable = C(R("az", "AZ")^1) / function(s) return { type = "variable", name = s } end,
    ...
    g.LagrangeNotation = (V"Atom" * C(P"'"^1)) / function(atom, primes)
      return { type = "derivative", notation = "lagrange", variable = atom, order = #primes }
    end

    g.NewtonDot = P"\\ddot" / function() return 2 end + P"\\dot" / function() return 1 end
    local braced_atom = mock_tk.space * P"{" * mock_tk.space * V"Atom" * mock_tk.space * P"}"
    local plain_atom = V"Atom"
    g.NewtonNotation = (V"NewtonDot" * (braced_atom + plain_atom)) / function(order, atom)
      return { type = "derivative", notation = "newton", variable = atom, order = order }
    end

    g.LeibnizNotation = (P"\\frac" * mock_tk.space * P"{" * mock_tk.space * P"d" * V"Atom" * mock_tk.space * P"}" *
                           mock_tk.space * P"{" * mock_tk.space * P"d" * V"Atom" * mock_tk.space * P"}") /
    function(dep, indep)
      return { type = "derivative", notation = "leibniz", dependent_variable = dep, independent_variable = indep, order = 1 }
    end
    ...
    g.Expression = Cf(
      V"Term" * (mock_tk.space * Ct(C(S"+-") * mock_tk.space * V"Term"))^0,
      function(acc, pair) return package.loaded["tungsten.core.ast"].create_binary_operation_node(pair[1], acc, pair[2]) end
    )
  ...
  describe("Lagrange Notation", function()
    it("should parse y' = y", function()
      ...
      assert.are.same({
        type = "derivative", notation = "lagrange", order = 1, variable = { type = "variable", name = "y" }
      }, result.lhs)
    end)
  ...
  describe("Leibniz Notation", function()
      it("should parse \\frac{dy}{dx} = y", function()
          ...
          assert.is_table(result)
          assert.are.same({
              type = "derivative", notation = "leibniz", order = 1,
  ...
  describe("Newton Notation", function()
      it("should parse \\dot{x} = x", function()
          ...
          assert.is_table(result)
          assert.are.same({
              type = "derivative", notation = "newton", order = 1, variable = { type = "variable", name = "x" }
          }, result.lhs)
      end)
    ...
    describe("Complex Equations", function()
        it("should parse derivatives on both sides: y' = y''", function()
            ...
            assert.are.same({
                type = "derivative", notation = "lagrange", order = 2, variable = { type = "variable", name = "y" }
            }, result.rhs)
        end)
        ...
        it("should parse a mix of notations: \\ddot{x} = \\frac{dy}{dt}", function()
            ...
            assert.is_table(result)
            assert.are.same({
                type = "derivative", notation = "newton", order = 2, variable = { type = "variable", name = "x"}
            }, result.lhs)
            assert.are.same({
                type = "derivative", notation = "leibniz", order = 1,
                dependent_variable = { type = "variable", name = "y"},
