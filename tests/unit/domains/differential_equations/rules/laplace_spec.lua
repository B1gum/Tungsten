...
describe("Differential Equations Laplace Rule", function()
	...
	it("should parse a Laplace transform with \\left\\{\\right\\} braces and whitespace", function()
		local result = parse_input("\\mathcal{L} \\left\\{  x  \\right\\}")
		assert.is_table(result)
		assert.are.same("laplace_transform", result.type)
		assert.is_table(result.expression)
		assert.are.same("variable", result.expression.type)
		assert.are.same("x", result.expression.name)
	end)

	it("should parse a Laplace transform using braces with newlines", function()
		local result = parse_input("\\mathcal{L}\\{\n\tf(t)\n\\}")
		assert.is_table(result)
		assert.are.same("laplace_transform", result.type)
		assert.is_table(result.expression)
		assert.are.same("function_call", result.expression.type)
	...
	it("should parse an inverse Laplace transform around a number", function()
		local result = parse_input("\\mathcal{L}^{-1}(42)")
		assert.is_table(result)
		assert.are.same("inverse_laplace_transform", result.type)
		assert.is_table(result.expression)
		assert.are.same("number", result.expression.type)
		assert.are.same(42, result.expression.value)
	end)
end)
