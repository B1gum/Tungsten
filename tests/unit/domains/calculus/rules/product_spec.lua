-- tests/unit/domains/calculus/rules/product_spec.lua
...
local modules_to_reset = {
	"tungsten.domains.calculus.rules.product",
...
describe("Calculus Product Rule: tungsten.domains.calculus.rules.product", function()
	before_each(function()
		...
		mock_ast_module = {
			...
			create_product_node = function(index_variable, start_expression, end_expression, body_expression)
				return {
					type = "product",
		...
		ProductRule = require("tungsten.domains.calculus.rules.product")

		test_grammar_table_definition = {
			"TestEntryPoint",
			TestEntryPoint = ProductRule * -P(1),
	...
	describe("Basic Product Parsing", function()
		it("should parse \\prod_{i=0}^{N} i^2 (unbraced body)", function()
			local input = "\\prod_{i=0}^{N} i^2"
			local expected_ast = {
				type = "product",
		...
		it("should parse \\prod_{k=1}^{10} {k^3} (braced body)", function()
			local input = "\\prod_{k=1}^{10} {k^3}"
			local expected_ast = {
				type = "product",
		...
		it("should parse with variable start/end: \\prod_{n=m}^{M} 1/n", function()
			local input = "\\prod_{n=m}^{M} 1/n"
			local expected_ast = {
				type = "product",
		...
		it("should parse infinite upper bounds: \\prod_{k=1}^{\\infty} k^3", function()
			local input = "\\prod_{k=1}^{\\infty} k^3"
			local expected_ast = {
				type = "product",
				index_variable = { type = "variable", name = "k" },
				start_expression = { type = "number", value = 1 },
		...
		it("should parse with Greek letter index: \\prod_{\\alpha=1}^{5} a_k", function()
			local input = "\\prod_{alpha=1}^{5} a_k"
			local expected_ast = {
				type = "product",
	...
	describe("Spacing Variations", function()
		it("should parse with extra spaces: \\prod _ { i = 0 } ^ { N } { i^2 }", function()
			local input = "\\prod _ { i = 0 } ^ { N } { i^2 }"
			local expected_ast = {
				type = "product",
				index_variable = { type = "variable", name = "i" },
				start_expression = { type = "number", value = 0 },
		...
		it("should parse with minimal spaces: \\prod_{i=0}^{N}i^2", function()
			local input = "\\prod_{i=0}^{N}i^2"
			local expected_ast = {
				type = "product",
				index_variable = { type = "variable", name = "i" },
	...
	describe("Invalid Syntax", function()
		it("should not parse if \\prod is missing: _{i=0}^{N} i^2", function()
			assert.is_nil(parse_input("_{i=0}^{N} i^2"))
		end)
		it("should not parse if subscript is missing: \\prod^{N} i^2", function()
			assert.is_nil(parse_input("\\prod^{N} i^2"))
		end)
		it("should not parse if superscript is missing: \\prod_{i=0} i^2", function()
			assert.is_nil(parse_input("\\prod_{i=0} i^2"))
		end)
		it("should not parse if '=' is missing in subscript: \\prod_{i 0}^{N} i^2", function()
			assert.is_nil(parse_input("\\prod_{i 0}^{N} i^2"))
		end)
		it("should not parse if start expression is missing: \\prod_{i=}^{N} i^2", function()
			assert.is_nil(parse_input("\\prod_{i=}^{N} i^2"))
		end)
		it("should not parse if index variable is missing: \\prod_{=0}^{N} i^2", function()
			assert.is_nil(parse_input("\\prod_{=0}^{N} i^2"))
		end)
		it("should not parse if end expression is missing: \\prod_{i=0}^{} i^2", function()
			assert.is_nil(parse_input("\\prod_{i=0}^{} i^2"))
		end)
		it("should not parse if body expression is missing: \\prod_{i=0}^{N}", function()
			assert.is_nil(parse_input("\\prod_{i=0}^{N}"))
		end)
		it("should not parse if subscript braces are mismatched: \\prod_{i=0^{N} i^2", function()
			assert.is_nil(parse_input("\\prod_{i=0^{N} i^2"))
		end)
		it("should not parse if superscript braces are mismatched: \\prod_{i=0}^{N {i^2}", function()
			assert.is_nil(parse_input("\\prod_{i=0}^{N {i^2}"))
		end)
	end)
end)
