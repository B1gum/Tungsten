...
describe("Calculus Partial Derivative Rule: tungsten.domains.calculus.rules.partial_derivatives", function()
	before_each(function()
		...
		test_grammar_table_definition = {
			...
			AtomBase = (mock_tokenizer_module.number + mock_tokenizer_module.variable),
			Expression = (P("f(x,y)") / function()
				return placeholder_node("placeholder_expr", "f(x,y)", "func_fxy")
			end + P("g(x)") / function()
				return placeholder_node("placeholder_expr", "g(x)", "func_gx")
			end + P("x^2y") / function()
				return placeholder_node("placeholder_expr", "x^2y", "term_x2y")
			end + V("AtomBase")),
		}
	...
	describe("Higher-order Overall (Numerator \\partial^ORDER)", function()
		...
		it(
			"should parse \\frac{\\partial^{2}}{\\partial x \\partial y} f(x,y) (mixed partial, overall order matches sum)",
			function()
		...
		it(
			"should parse from prompt: \\frac{\\partial^{<ORDER>}}{\\partial <VARIABLE> \\partial <VARIABLE>} <EXPR> (orders 1 in den)",
			function()
	...
	describe("Invalid Syntax", function()
		...
		it(
			"should not parse if \\partial is missing in a denominator term: \\frac{\\partial}{\\partial x y^2} f(x,y)",
			function()
