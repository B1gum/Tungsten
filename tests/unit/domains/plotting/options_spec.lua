...
describe("Plotting Options and Defaults", function()
	...
	it("allows overriding legend position", function()
		local opts = options_builder.build({ dim = 2, form = "explicit" }, { legend_pos = "upper left" })
		assert.are.same("upper left", opts.legend_pos)
	end)
	...
	describe("legend_auto", function()
		it("is nil by default", function()
			local opts = options_builder.build({ dim = 2, form = "explicit" }, {})
			assert.is_nil(opts.legend_auto)
		end)

		it("preserves classification value", function()
			local opts = options_builder.build({ dim = 2, form = "explicit", legend_auto = false }, {})
			assert.is_false(opts.legend_auto)
		end)

		it("allows overrides to take precedence", function()
			local opts = options_builder.build({ dim = 2, form = "explicit", legend_auto = false }, { legend_auto = true })
