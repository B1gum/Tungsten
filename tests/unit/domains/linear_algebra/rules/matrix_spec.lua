...
describe("Linear Algebra Matrix Rule: tungsten.domains.linear_algebra.rules.matrix", function()
	before_each(function()
		...
		mock_tokenizer_module = {
			...
			ampersand = P("&") / function()
				return { type = "ampersand" }
			end,
			double_backslash = P("\\\\") / function()
				return { type = "double_backslash" }
			end,
			number = C(R("09") ^ 1) / function(s)
				return placeholder_expr_node("num:" .. s)
			end,
			variable = C(R("az") ^ 1) / function(s)
				return placeholder_expr_node("var:" .. s)
			end,
		}
	...
	describe("Invalid Matrix Structures", function()
		...
		it(
			"should not parse if a row separator is missing between rows: \\begin{pmatrix} 1 \\\\ 2 3 \\end{pmatrix}",
			function()
