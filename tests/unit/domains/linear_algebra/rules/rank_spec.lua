...
local P, V, C, R, S, Cg, Ct = lpeg.P, lpeg.V, lpeg.C, lpeg.R, lpeg.S, lpeg.Cg, lpeg.Ct
...
describe("Linear Algebra Rank Rule: tungsten.domains.linear_algebra.rules.rank", function()
  ...
  before_each(function()
    ...
    mock_tokenizer_module = {
      ...
      matrix_placeholder_rule = P("\\begin{pmatrix}") * C((P(1) - P("\\end{pmatrix}"))^0) * P("\\end{pmatrix}") /
                                function(matrix_content_str) return placeholder_expr_node(matrix_content_str, "matrix") end,
    }
