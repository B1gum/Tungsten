  ...
  before_each(function()
    ...
    mock_tokenizer_module = {
      ...
      variable = C(R("AZ","az") * (R("AZ","az","09")^0)) / function(s)
        ...
        if s == "intercal" then return {type="variable", name="intercal"} end
    ...
    local exponent_rule =
      ( mock_tokenizer_module.lbrace * mock_tokenizer_module.space *
        exponent_content_ast_producer *
        mock_tokenizer_module.space * mock_tokenizer_module.rbrace
      ) +
      exponent_content_ast_producer

    local mock_postfix_op =
      ( P("^") * mock_tokenizer_module.space *
        exponent_rule /
      ...
      ( P("_") * mock_tokenizer_module.space *
        exponent_rule /
    ...
    local mock_supsub_base_elements = mock_tokenizer_module.variable +
                                  ...
                                  (mock_tokenizer_module.lparen * C(P"A+B") * mock_tokenizer_module.rparen / function(c_str) return base_node(c_str, "group") end) +
                                  (P"\\mymatrix" / function() return matrix_node_placeholder("mymatrix") end)

    local mock_supsub_rule = Cf(mock_supsub_base_elements * (mock_tokenizer_module.space * mock_postfix_op)^0,
      function(acc_ast, op_builder_func)
        if op_builder_func then return op_builder_func(acc_ast) end
        return acc_ast
      end
    )
