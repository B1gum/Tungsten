...
describe("Linear Algebra Determinant Rule: tungsten.domains.linear_algebra.rules.determinant", function()
  ...
  before_each(function()
    ...
    mock_tokenizer_module = {
      space = S(" \t\n\r")^0,
      det_command = P("\\det") / function() return { type = "det_command_token" } end,
      vbar = P("|") * -P("|") / function() return { type = "vbar_token" } end,
      lparen = P("("),
      rparen = P(")"),
    ...
    local matrix_rule_placeholder = P("\\begin{pmatrix}") * (mock_tokenizer_module.variable + mock_tokenizer_module.number)^0 * P("\\end{pmatrix}") / function() return matrix_node("pmatrix") end
