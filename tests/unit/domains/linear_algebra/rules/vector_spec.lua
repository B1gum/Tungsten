...
describe("Linear Algebra Vector Rule (Symbolic): tungsten.domains.linear_algebra.rules.vector", function()
	before_each(function()
		...
		mock_tokenizer_module = {
			...
			lbrace = lpeg_P("{"),
			rbrace = lpeg_P("}"),
			variable = lpeg_C(lpeg_R("az", "AZ") * (lpeg_R("az", "AZ", "09") ^ 0)) / function(v_str)
				return variable_node(v_str)
			end,
			number = lpeg_C(lpeg_R("09") ^ 1) / function(n_str)
				return number_node(tonumber(n_str))
			end,

			matrix_env_name_capture = lpeg_C(lpeg_P("pmatrix") + lpeg_P("bmatrix") + lpeg_P("vmatrix")),
			ampersand = lpeg_P("&") / function()
				return { type = "ampersand_token_from_mock" }
			end,
			double_backslash = lpeg_P("\\\\") / function()
				return { type = "double_backslash_token_from_mock" }
			end,
		}
		...
		test_grammar_table_definition = {
			...
			Expression = (
				...
				* lpeg_P("_")
				* local_mock_space
				* (mock_tokenizer_module.variable + mock_tokenizer_module.number)
				/ function(b, s)
					return mock_ast_module.create_subscript_node(b, s)
				end
			)
				+ mock_tokenizer_module.variable
