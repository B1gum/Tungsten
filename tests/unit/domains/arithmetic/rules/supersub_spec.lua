...
describe("Arithmetic SupSub Rule: tungsten.domains.arithmetic.rules.supersub", function()
	before_each(function()
		...
		mock_tokenizer_module = {
			...
			variable = C(R("AZ", "az") * (R("AZ", "az", "09") ^ 0)) / function(s)
				...
				if s == "intercal" then
					return { type = "intercal_command" }
				end
				return base_node(s, "variable")
			end,
			number = C((P("-") ^ -1 * R("09") ^ 1 * (P(".") * R("09") ^ 1) ^ -1)) / function(s)
				return base_node(tonumber(s), "number")
			end,
			matrix_placeholder = P("Matrix") * C(R("AZ")) / function(id)
				return base_node(id, "matrix")
			end,
			vector_placeholder = P("\\vec{") * C(R("az")) * P("}") / function(name)
				return base_node(name, "symbolic_vector")
			end,
		}
		...
		mock_ast_module = {
			...
			create_binary_operation_node = function(op, l, r)
		...
		local exponent_token_content = mock_tokenizer_module.variable
			...
			+ (P("expComplex") / function()
				return { type = "complex_exponent_placeholder" }
			end)
		...
		test_grammar_table_definition = {
			...
			AtomBase = mock_tokenizer_module.matrix_placeholder
				...
				+ (mock_tokenizer_module.lparen * C(R("az")) * P("+") * C(R("az")) * mock_tokenizer_module.rparen / function(
					v1,
					v2
				)
					return base_node(v1 .. "+" .. v2, "group_placeholder")
				end)
	...
	describe("Valid Transpose Operations", function()
		...
		it("should parse \\vec{v}^T as superscript", function()
			local input = "\\vec{v}^T"
			local expected =
				mock_ast_module.create_superscript_node(base_node("v", "symbolic_vector"), base_node("T", "variable"))
			assert.are.same(expected, parse_input(input))
		end)
		...
		it("should parse (a+b)^T for a grouped expression (assuming group is matrix-like)", function()
			...
			test_grammar_table_definition.AtomBase = (
				mock_tokenizer_module.lparen
				* C(R("az"))
				* P("+")
				* C(R("az"))
				* mock_tokenizer_module.rparen
				/ function(v1, v2)
					return base_node(v1 .. "+" .. v2, "matrix")
				end
			)
	...
	describe("Standard Superscript/Power Operations", function()
		...
		it("should parse (a+b)^n as superscript", function()
			local input = "(a+b)^n"
			local expected =
				mock_ast_module.create_superscript_node(base_node("a+b", "group_placeholder"), base_node("n", "variable"))
			assert.are.same(expected, parse_input(input))
		end)
		...
		it("should parse \\vec{v}^2 as superscript (vector to power)", function()
			local input = "\\vec{v}^2"
			local expected =
				mock_ast_module.create_superscript_node(base_node("v", "symbolic_vector"), base_node(2, "number"))
			assert.are.same(expected, parse_input(input))
		end)
	end)
	...
	describe("Standard Subscript Operations", function()
		...
		it("should parse \\vec{v}_{j} as subscript", function()
			...
			local expected =
				mock_ast_module.create_subscript_node(base_node("v", "symbolic_vector"), base_node("j", "variable"))
			assert.are.same(expected, parse_input(input))
		end)
	end)
	...
	describe("Edge Cases and Invalid Syntax", function()
		...
		it("should not parse MatrixA^-T (invalid exponent for inverse/transpose)", function()
			local original_atom_base = test_grammar_table_definition.AtomBase
			local exponent_token_content = (P("-T") / function()
				return { type = "negated_T_literal" }
			end) + mock_tokenizer_module.variable + mock_tokenizer_module.number
			test_grammar_table_definition.AtomBase = mock_tokenizer_module.matrix_placeholder
				+ mock_tokenizer_module.vector_placeholder
				+ (mock_tokenizer_module.lbrace * mock_tokenizer_module.space * exponent_token_content * mock_tokenizer_module.space * mock_tokenizer_module.rbrace)
				+ exponent_token_content
			compiled_test_grammar = P(test_grammar_table_definition)
