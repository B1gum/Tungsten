...
local P, R, S, C, Cf, Ct, V = lpeg.P, lpeg.R, lpeg.S, lpeg.C, lpeg.Cf, lpeg.Ct, lpeg.V
...
local mock_tk_original = {
  ...
  vec_command = P("\\vec{") * C(R("az", "AZ", "09")^1) * P("}") / function(s) return { type = "symbolic_vector", name_expr = { type = "variable", name = s} } end,
  matrix_placeholder = P"Matrix" * C(R("AZ")) / function(id) return { type = "matrix", id = id } end,
  times_command = P"\\times" / function() return "\\times" end,
  cdot_command = P"\\cdot" / function() return "\\cdot" end,
}
...
local function compile_test_grammar_muldiv(rule_to_test)
  ...
  local ExpressionForParensDef = ( V("AtomForUnary") * mock_tk_original.space * P("+") * mock_tk_original.space * V("AtomForUnary") /
                                  function(l,r) return mock_ast_utils_original.create_binary_operation_node("+", l,r) end) +
                                 ( V("AtomForUnary") * P("^") * V("AtomForUnary") / function(b_val,e_val) return mock_ast_utils_original.create_superscript_node(b_val,e_val) end ) +
                                 rule_to_test
...
describe("Arithmetic MulDiv Rule (Context-Aware & Differentials)", function()
  ...
  before_each(function()
    ...
    MockUnaryRule_MulDiv = V("AtomForUnary") +
                           (C(S("+-")) * mock_tk_original.space * V("AtomForUnary") / function(op, val)
    ...
    package.loaded["tungsten.domains.arithmetic.rules.supersub"] = modules_to_mock_muldiv["tungsten.domains.arithmetic.rules.supersub"]
  ...
  describe("Context-Aware Cross Product", function()
    ...
    it("should parse MatrixA \\times MatrixB as cross_product (if matrices are considered vector-like by rule)", function()
  ...
  describe("Standard Multiplication, Division, and Implicit (Regression Tests)", function()
    it("should still parse 2*var as binary multiplication", function()
      ...
      assert.are.same({ type = "binary", operator = "*",
        left = { type = "number", value = 2 },
  ...
  describe("Precedence with AddSub (Combined Operations)", function()
    ...
    it("should correctly parse x^2 \\cdot \\vec{y} as binary '*' (scalar times vector)", function()
        ...
        MockUnaryRule_MulDiv = (V("AtomForUnary") * P("^") * V("AtomForUnary") / function(b,e)
