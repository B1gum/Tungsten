...
describe("Tungsten Persistent Variable Pipeline", function()
	...
	after_each(function()
		...
		if mock_parser_parse_spy and mock_parser_parse_spy.clear then
			mock_parser_parse_spy:clear()
		end
		if mock_ast_to_wolfram_spy and mock_ast_to_wolfram_spy.clear then
			mock_ast_to_wolfram_spy:clear()
		end
	...
	describe("Full Persistent Variable Definition and Evaluation Pipeline", function()
		...
		it("should define multiple variables and use them in a dependent evaluation", function()
			...
			assert.spy(mock_async_run_job_spy).was.called(1)
			local cmd_args = mock_async_run_job_spy.calls[1].vals[1]
			local expected_wolfram_code =
				'ToString[TeXForm[wolfram((wolfram((wolfram(5)) + 1)) * 2)], CharacterEncoding -> "UTF8"]'
		...
		it("should respect 'persistent_variable_assignment_operator' from config ('=')", function()
			...
			local cmd_args = mock_async_run_job_spy.calls[1].vals[1]
			assert.are.same(
				'ToString[TeXForm[wolfram((wolfram(3)) * (wolfram(3)))], CharacterEncoding -> "UTF8"]',
				cmd_args[3]
			)
		...
		it("should respect 'persistent_variable_assignment_operator' from config (':=')", function()
			...
			local cmd_args = mock_async_run_job_spy.calls[1].vals[1]
			assert.are.same(
				'ToString[TeXForm[wolfram((wolfram(4)) + (wolfram(4)))], CharacterEncoding -> "UTF8"]',
				cmd_args[3]
			)
