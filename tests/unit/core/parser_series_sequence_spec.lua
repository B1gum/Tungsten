...
describe("parser series and sequence handling", function()
	...
	it("splits input into multiple series by semicolon", function()
		local res = parser.parse("(1,2); (3,4); x^2 + y^2 = 1")
	...
	it("handles '.' and '|' delimiters in \\left...\\right", function()
		...
		local parts = lexer.split_top_level("\\left.1,2\\right|, 3", { [","] = true })
		assert.are.equal(2, #parts)
		assert.are.same("\\left1,2\\right|", parts[1].str)
		assert.are.same(" 3", parts[2].str)
	end)

	it("handles mismatched \\left/\\right delimiters", function()
		local lexer = require("tungsten.core.lexer")
		local parts = lexer.split_top_level("\\left(1\\right],2", { [","] = true })
		assert.are.equal(2, #parts)
		assert.are.same("\\left(1\\right]", parts[1].str)
		assert.are.same("2", parts[2].str)
	end)
	...
	it("does not collapse mixed equations into a system capture", function()
		local res = parser.parse("x=1; y=2; z+1", { allow_multiple_relations = true })
		assert.is_not_nil(res)
		assert.are.equal(3, #res.series)
		assert.are.equal("Equality", res.series[1].type)
		assert.are.equal("Equality", res.series[2].type)
		assert.are.equal("binary", res.series[3].type)
	end)
end)
