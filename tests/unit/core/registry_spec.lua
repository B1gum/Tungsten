	...
	describe("M.get_combined_grammar()", function()
		...
		it("Rule Overriding: Higher priority domain's rule overrides lower priority for same name, logs debug", function()
			...
			registry_module.register_grammar_contribution(
				...
				50,
				"MyRule",
				create_mock_pattern("low_pattern"),
				"MyCategory"
			)
			registry_module.register_grammar_contribution(
				"HighPrioDomain",
				100,
				"MyRule",
				create_mock_pattern("high_pattern"),
				"MyCategory"
			)
			registry_module.register_grammar_contribution(
				"core",
			...
			local expected_log_msg = ("Registry: Rule '%s': %s (Prio %d) NOT overriding existing from %s (Prio %d)."):format(
				"MyRule",
				"LowPrioDomain",
				50,
				"HighPrioDomain",
			...
			for _, call in ipairs(mock_logger_notify_spy.calls) do
				if
					call.vals[1] == expected_log_msg
					and call.vals[2] == mock_logger_module.levels.DEBUG
					and call.vals[3].title == "Tungsten Registry"
				then
					log_found = true
					break
				end
			...
			assert.is_true(
				log_found,
				"Expected log for LowPrio NOT overriding HighPrio not found. Expected: '"
					.. expected_log_msg
					.. "'. Actual logs: "
					.. actual_logs
			)
		...
		it("Conflict Handling (Same Priority): One takes precedence due to sort order, logs warning", function()
			...
			for _, call in ipairs(mock_logger_notify_spy.calls) do
				if
					call.vals[1] == expected_conflict_log
					and call.vals[2] == mock_logger_module.levels.WARN
					and call.vals[3].title == "Tungsten Registry Conflict"
				then
			...
			assert.is_true(
				conflict_log_found,
				"Expected conflict log for DomainB winning not found. Expected: '"
					.. expected_conflict_log
					.. "'. Actual logs: "
					.. actual_logs_for_debug
			)
			mock_config_module.debug = false
		end)
