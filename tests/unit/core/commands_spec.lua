...
describe("Tungsten core commands", function()
  ...
  before_each(function()
    ...
    _G.require = function(module_path)
      ...
      if module_path == 'tungsten.util.commands' then return mock_cmd_utils_module end
    ...
    eval_async_behaviors.nil_eval = function(_, _, callback) callback(nil) end
    eval_async_behaviors.empty_string_eval = function(_, _, callback) callback("") end
    ...
    mock_wolfram_backend_module.ast_to_wolfram = spy.new(function(ast) return "wolfram(".. (ast.name or ast.id) ..")" end)
  ...
  describe(":TungstenSolve", function()
    ...
    it("should call insert_result when solver callback provides a solution", function()
        ...
        assert.spy(mock_insert_result_insert_result_spy).was.called_with("some_solution", " \\rightarrow ", match.is_table(), match.is_table(), "a*x^2+b*x+c=0")
    end)
  end)
  ...
  describe(":TungstenStatus", function()
    it("calls engine.get_active_jobs_summary and logs", function()
      mock_evaluator_module.get_active_jobs_summary = function() return "status" end
