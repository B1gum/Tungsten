...
describe("Registry priority override", function()
	...
	before_each(function()
		...
		logger.info = function(t, m)
			logger_notify_spy(m, logger.levels.INFO, { title = t })
		end
		logger.warn = function(t, m)
			logger_notify_spy(m, logger.levels.WARN, { title = t })
		end
		logger.error = function(t, m)
		...
		_G.require = function(module_path)
			...
			if mock_modules[module_path] then
	...
	it("uses handler from domain with higher user priority", function()
		...
		local result = wolfram_backend.ast_to_code({ type = "op" })
