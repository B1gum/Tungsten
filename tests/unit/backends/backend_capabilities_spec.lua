...
describe("backend capabilities", function()
	it("returns capabilities for each backend", function()
		...
		assert.is_true(wolfram.supports.inequalities[2])
		assert.is_nil(wolfram.supports.points[2])
		...
		assert.is_nil(python.supports.inequalities[2])
		assert.is_nil(python.supports.points[2])
	end)
	...
	it("checks support correctly", function()
		...
		assert.is_false(backends.is_supported("wolfram", "explicit", 2, { points = true }))
		assert.is_false(backends.is_supported("python", "explicit", 2, { points = true }))
	end)
