...
describe("Tungsten Linear Algebra Domain Wolfram Handlers", function()
	...
	describe("cross_product handler", function()
		it("should correctly format Cross[vec1, vec2]", function()
			...
			assert.are.equal("Cross[Flatten[a], Flatten[b]]", result)
			assert.spy(mock_recur_render).was.called_with(vec1_node)
			assert.spy(mock_recur_render).was.called_with(vec2_node)
		end)
	end)
	...
	describe("zero_vector_matrix handler", function()
		...
		it("should correctly format ConstantArray[0, {m, n}] for a matrix", function()
			...
			mock_recur_render = spy.new(function(child_node)
				if child_node == dim_node_for_matrix then
					return "{2, 3}"
				end
	...
	describe("eigenvectors handler", function()
		it("should correctly format Eigenvectors[matrix_representation]", function()
			local matrix_expr_node = ast_node("matrix_placeholder", { name = "MyTestMatrix" })
			local node = ast_node("eigenvectors", { expression = matrix_expr_node })

			local result = handlers.eigenvectors(node, mock_recur_render)
			assert.are.equal("Eigenvectors[MyTestMatrix]", result)
			assert.spy(mock_recur_render).was.called_with(matrix_expr_node)
		end)
	end)

	describe("eigensystem handler", function()
		it("should correctly format Eigensystem[matrix_representation]", function()
			local matrix_expr_node = ast_node("matrix_placeholder", { name = "MyEigenMatrix" })
			local node = ast_node("eigensystem", { expression = matrix_expr_node })

			local result = handlers.eigensystem(node, mock_recur_render)
			assert.are.equal("Eigensystem[MyEigenMatrix]", result)
			assert.spy(mock_recur_render).was.called_with(matrix_expr_node)
		end)
	end)
end)
