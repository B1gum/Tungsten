...
describe("tungsten.backends.wolfram (Plenary Env)", function()
	...
	before_each(function()
		...
		if tungsten_registry_module then
			...
			get_domain_priority_spy = spy.new(function()
				return 0
			end)
			...
			if spy_obj_type == "table" then
				local keys = {}
				for k, v_type in pairs(get_domain_priority_spy) do
					table.insert(keys, k .. " (type: " .. type(v_type) .. ")")
				end
			elseif spy_obj_type == "function" then
				local mt = getmetatable(get_domain_priority_spy)
				if mt then
					local mt_keys = {}
					for k, v_type in pairs(mt) do
						table.insert(mt_keys, k .. " (type: " .. type(v_type) .. ")")
					end
				end
			end
					...
					if handlers_table and type(handlers_table) == "table" then
						...
						if actual_handler_to_call then
							if type(actual_handler_to_call) == "table" or type(actual_handler_to_call) == "function" then
								local result_val = actual_handler_to_call(ast_node, function()
									return "dummy_recursive_call_output_from_render_spy"
								end)
								return result_val
							else
		...
		if tungsten_logger_module then
			...
			tungsten_logger_module.debug = function(t, m)
				logger_notify_spy(m, tungsten_logger_module.levels.DEBUG, { title = t })
			end
			tungsten_logger_module.info = function(t, m)
				logger_notify_spy(m, tungsten_logger_module.levels.INFO, { title = t })
			end
			tungsten_logger_module.warn = function(t, m)
				logger_notify_spy(m, tungsten_logger_module.levels.WARN, { title = t })
			end
			tungsten_logger_module.error = function(t, m)
				logger_notify_spy(m, tungsten_logger_module.levels.ERROR, { title = t })
			end
		else
		...
		assert(
			get_domain_priority_spy and type(get_domain_priority_spy.callback) == "function",
			"Spy for 'get_domain_priority_spy' MUST have a .callback function. Spy Type: "
				.. type(get_domain_priority_spy)
				.. ", .callback type: "
				.. type(get_domain_priority_spy and get_domain_priority_spy.callback)
		)
		...
		local main_priority_logic_func = function(domain_name)
			...
			if current_priority_mode == "conflict_test" then
				if received_domain_name_str == "custom_domain_one" then
					priority_to_return = 200
				elseif received_domain_name_str == "high_prio_domain" then
					priority_to_return = 200
				elseif received_domain_name_str == "custom_domain_two" then
					priority_to_return = 90
				elseif received_domain_name_str == "arithmetic" then
					priority_to_return = 100
				else
					priority_to_return = 0
				end
			else
				if received_domain_name_str == "arithmetic" then
					priority_to_return = 100
				elseif received_domain_name_str == "custom_domain_one" then
					priority_to_return = 110
				elseif received_domain_name_str == "custom_domain_two" then
					priority_to_return = 90
				elseif received_domain_name_str == "high_prio_domain" then
					priority_to_return = 200
				elseif received_domain_name_str == "low_prio_domain" then
					priority_to_return = 50
				elseif received_domain_name_str == "successful_domain" then
					priority_to_return = 120
				elseif received_domain_name_str == "missing_domain" then
					priority_to_return = 10
				elseif received_domain_name_str == "nohandlers_domain" then
					priority_to_return = 20
				elseif received_domain_name_str == "another_good_domain" then
					priority_to_return = 130
				elseif received_domain_name_str == "another_good_domain_for_nohandlers_test" then
					priority_to_return = 40
				elseif received_domain_name_str == "nil_return_domain" then
					priority_to_return = 25
				else
					priority_to_return = 0
				end
			end
		...
		if not (render_render_spy and type(render_render_spy.callback) == "function") then
			error(
				string.format(
					"Spy for 'render_render_spy' does not have a .callback method. Spy type: %s, .callback type: %s. Available keys: %s",
			...
			if ast_node and ast_node.type then
				if handlers_table then
					...
					if handler_func_from_table then
						...
						local result = actual_handler_to_call(ast_node, function()
							return "dummy_recursive_call_output"
						end)
						return result
					end
				end
		...
		_G.require = function(module_path)
			...
			if module_path == "tungsten.core.registry" then
	...
	describe("Default Handler Loading (config.domains is nil or empty)", function()
		it("should attempt to load arithmetic handlers if config.domains is nil", function()
			...
			assert.is_true(
				found_arithmetic_handler_require,
				"Expected _G.require to be called for 'tungsten.backends.wolfram.domains.arithmetic'"
			)
	...
	describe("Custom Domain Handler Loading", function()
		it("should attempt to load handlers from a single domain specified in config.domains", function()
			...
			assert.is_true(
				found_custom_handler_require,
				"Expected _G.require to be called for '" .. handler_module_path .. "'"
			)
			...
			assert.is_false(
				found_arithmetic_handler_require,
				"Did not expect _G.require for 'arithmetic' when not in config.domains and custom domains are specified."
			)
		end)
		...
		it("should attempt to load handlers from multiple domains specified in config.domains", function()
			...
			assert.is_true(
				require_calls_for_custom_domains[handler_module_path_one],
				"Expected _G.require for '" .. handler_module_path_one .. "'"
			)
			assert.is_true(
				require_calls_for_custom_domains[handler_module_path_two],
				"Expected _G.require for '" .. handler_module_path_two .. "'"
			)
		...
		it(
			"should also load arithmetic handlers if 'arithmetic' is explicitly in config.domains along with custom domains",
			function()
				...
				assert.is_true(
					called_modules_map[custom_handler_module_path],
					"Expected require for custom_domain_one handlers"
				)
				assert.spy(get_domain_priority_spy).was.called_with("arithmetic")
	...
	describe("Handler Prioritization and Overriding", function()
		...
		before_each(function()
			...
			low_prio_handler_spy_func = spy.new(function()
				return "low_prio_output"
			end)
			high_prio_handler_spy_func = spy.new(function()
				return "high_prio_output"
			end)
		...
		it(
			...
			function()
				...
				assert.is_true(
					not_override_log_found,
					"Expected debug log for 'NOT overriding' was not found. Logged: " .. simple_inspect(logger_notify_spy.calls)
				)
		...
		it("should log a warning and pick one for same priority conflict", function()
			...
			local handler_spy_high_prio = spy.new(function()
				return "high_prio_output"
			end)
			...
			assert.is_true(
				conflict_log_found,
				"Expected WARN log for handler conflict was not found or was incorrect. \nExpected: ["
					.. expected_log_message
					.. "]\nActual Logged Messages: "
					.. logged_calls_str
			)
	...
	describe("Handling of Domain Modules", function()
		it("should successfully load and process handlers from a correctly defined domain module", function()
			...
			local success_handler_spy_func = spy.new(function()
				return "success_domain_output"
			end)
			mock_domain_handler_definitions[handler_module_path] = {
			...
			local expected_log_message = ("Wolfram Backend: Successfully loaded handlers module from %s for domain %s (Priority: %d)"):format(
				handler_module_path,
				domain_name,
				domain_priority
			)
			...
			assert.is_true(
				success_log_found,
				"Expected debug log for successful handler loading not found. Logged: "
					.. simple_inspect(logger_notify_spy.calls)
			)
		...
		it("should log a warning and continue if a domain module is missing (require throws error)", function()
			...
			local good_handler_spy_func = spy.new(function()
				return "good_output"
			end)
			...
			local simulated_error_fragment = "Simulated error: module '"
				.. handler_module_path_missing
				.. "' not found by test override."
			local expected_log_message_pattern_start = ("Wolfram Backend: Could not load Wolfram handlers for domain '%s'. Failed to load module '%s': "):format(
				missing_domain_name,
				handler_module_path_missing
			)
			...
			for _, call_info in ipairs(logger_notify_spy.calls) do
				...
				if
					type(msg) == "string"
					and string.sub(msg, 1, #expected_log_message_pattern_start) == expected_log_message_pattern_start
					and string.find(msg, simulated_error_fragment, 1, true)
					and level == tungsten_logger_module.levels.WARN
					and opts
					and opts.title == "Tungsten Backend Warning"
				then
			...
			assert.is_true(
				warning_log_found,
				"Expected WARN log for missing domain module (erroring require) not found or incorrect. Logged: "
					.. simple_inspect(logger_notify_spy.calls)
			)
		...
		it("should log a warning if a domain module is loaded but does not have a .handlers table", function()
			...
			local good_handler_spy_func = spy.new(function()
				return "good_output_nohandlers_test"
			end)
			mock_domain_handler_definitions[handler_module_path_good] = {
			...
			local warning_log_found = false
			local expected_log_message = ("Wolfram Backend: Could not load Wolfram handlers for domain '%s'. module '%s' loaded but it did not return a .handlers table."):format(
				nohandlers_domain_name,
				handler_module_path_nohandlers
			)
			for _, call_info in ipairs(logger_notify_spy.calls) do
				local msg, level, opts = call_info.vals[1], call_info.vals[2], call_info.vals[3]
				if
					msg == expected_log_message
					and level == tungsten_logger_module.levels.WARN
					and opts
					and opts.title == "Tungsten Backend Warning"
				then
			...
			assert.is_true(
				warning_log_found,
				"Expected WARN log for domain module without .handlers table not found. Logged: "
					.. simple_inspect(logger_notify_spy.calls)
			)
		...
		it(
			"should log a warning if a domain module loads as nil (e.g. require returns true, nil via mock_domain_handler_definitions)",
			function()
				...
				assert.is_true(
					warning_log_found,
					"Expected WARN log for domain module returning nil not found. Logged: "
						.. simple_inspect(logger_notify_spy.calls)
				)
			end
		)
