...
M.handlers = {
	...
	limit = function(node, recur_render)
		local expr_str, var_str, point_str =
			render_util.render_fields(node, { "expression", "variable", "point" }, recur_render)
		return ("sp.limit(%s, %s, %s)"):format(expr_str, var_str, point_str)
	end,

	indefinite_integral = function(node, recur_render)
		local integrand_str, var_str = render_util.render_fields(node, { "integrand", "variable" }, recur_render)
		return ("sp.integrate(%s, %s)"):format(integrand_str, var_str)
	end,

	definite_integral = function(node, recur_render)
		local integrand_str, var_str, lower_bound_str, upper_bound_str =
			render_util.render_fields(node, { "integrand", "variable", "lower_bound", "upper_bound" }, recur_render)
