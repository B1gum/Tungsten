...
local function is_vector_like(node)
	if not (node and node.type) then
		return false
	end
...
local ExplicitOpAndTerm = space
	* ExplicitOpCapture
	* space
	* Unary
	/ function(op_str, term_ast)
		return { operator_str = op_str, term = term_ast }
	end

local ImplicitMulAndTerm = space
	* -S("+-*/")
	* -P("\\cdot")
	* -P("\\times")
	* -is_potential_differential_start
	* Unary
	/ function(term_ast)
		return { operator_str = "*", term = term_ast, implicit = true }
	end

local MulDiv = Cf(Unary * (ExplicitOpAndTerm + ImplicitMulAndTerm) ^ 0, function(left_acc_ast, op_and_right_term)
