...
local function tungsten_linear_independent_command(_)
	...
	if
		parsed_ast.type ~= "matrix"
		and parsed_ast.type ~= "vector_list"
		and parsed_ast.type ~= "symbolic_vector"
		and parsed_ast.type ~= "vector"
	then
		error_handler.notify_error(
			"LinearIndependent",
			"Selected text is not a valid matrix or list of vectors. Parsed as: " .. parsed_ast.type
		)
		return
	end
...
local function tungsten_eigenvalue_command(_)
	...
	if matrix_ast_node.type ~= "matrix" then
		error_handler.notify_error(
			"Eigenvalue",
			"The selected text is not a valid matrix. Parsed as: " .. (matrix_ast_node and matrix_ast_node.type or "nil")
		)
		return
	end
