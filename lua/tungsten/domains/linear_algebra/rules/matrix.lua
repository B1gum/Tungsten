...
local MatrixRowWithSeparators = Ct(MatrixElement * (space * tk.ampersand * space * MatrixElement) ^ 0)
local MatrixRow = MatrixRowWithSeparators
	/ function(captures)
		local elements_only = {}
...
local MatrixBody = MatrixBodyWithSeparators
	/ function(captures)
		local rows_only = {}
		if type(captures) == "table" then
			for _, item in ipairs(captures) do
...
local MatrixRule = Ct(matrix_begin_pattern * space * MatrixBody * space * matrix_end_pattern)
	/ function(captures)
		local begin_env_type = captures[1]
		local actual_matrix_rows = captures[2]
		local end_env_type = captures[3]

		if not (type(begin_env_type) == "string" and begin_env_type ~= "") then
			return nil
		end
		if not (type(end_env_type) == "string" and end_env_type ~= "") then
			return nil
		end
		if begin_env_type ~= end_env_type then
			return nil
		end
		if type(actual_matrix_rows) ~= "table" then
			return nil
		end
