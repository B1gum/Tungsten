...
local function walk(node)
  ...
  if node.type == 'superscript' then
    ...
    local base_can_be_inverted = base and (base.type == 'matrix' or base.type == 'symbolic_vector' or base.type == 'vector')
    if base_can_be_inverted and exp then
      if exp.type == 'number' and exp.value == -1 then
        return ast.create_inverse_node(base)
      end
      if exp.type == 'unary' and exp.operator == '-' and exp.value and exp.value.type == 'number' and exp.value.value == 1 then
        return ast.create_inverse_node(base)
      end
    end
  end
