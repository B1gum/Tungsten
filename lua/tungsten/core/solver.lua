    ...
    function(exit_code, final_stdout, final_stderr)
        if exit_code == 0 then
            ...
            elseif exit_code == 0 then 
                ...
                reason = "exited with code " .. tostring(exit_code)
            end

            local err_msg
            if exit_code < 1 then
                 err_msg = string.format("TungstenSolve: Failed to start WolframScript job for solving. (Reason: %s)", reason)

            else
                 err_msg = string.format("TungstenSolve: WolframScript (Job N/A) error. Code: %s\nStderr: %s\nStdout: %s",
